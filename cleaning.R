...
# Checking coding of variables -------------------------------------------------
## Dummy coding with first target as reference as wanted
contrasts(long_data$target_number_collapsed) 
...
levels(long_data$target_group)
long_data$target_group <- relevel(long_data$target_group, "Trump Supporter Target")
contrasts(long_data$target_group) <- contr.sum(2)
