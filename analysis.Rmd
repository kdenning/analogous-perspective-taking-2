...
model_base <- lmer(target_bfi_value ~ self_c*target_number_collapsed + (1|target_number_collapsed), data = model_data)
```
...
# Level 2 set-up
randeff <- ranef(model_base)$target_number_collapsed

names(randeff) <- c("u0", "u1") #random effects
randeff$target_number_collapsed <- rownames(randeff)

model_data <- merge(model_data, randeff, by="target_number_collapsed")
```
...
Why are there three bars at 1? There are two groups for target number collapsed, so there should be two...
...
Line relatively flat, looks equally distributed, even if weird due to the variable.

### Level 2: Targets/Time

#### Normality of residuals for intercept

```{r}
hist(model_data$u0, prob = T)
```

#### Heteroscedasticity 

```{r}
plot(model_data$u0 ~ model_data$target_number_collapsed)
```

Seems difficult to look at this with only two time points at L2; At completely different spots....

#### Normality of residuals for RE

```{r}
hist(model_data$u1, prob = T)
```

#### Heteroscedasticity 

```{r}
plot(model_data$u1 ~ model_data$target_number_collapsed)
```

Same as for the intercept
