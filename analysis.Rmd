...
### Checking if post intervention (target_2) is significantly different from 0

```{r out diff from zero, echo = FALSE}
diff0_1 <- model_base <- lmer(target_bfi_value ~ self_c + (self_c|sub_id), data = model_data, subset = (model_data$target_number_collapsed == 'target_2'))

tab_model(diff0_1,
          title = "Testing if post effect differs from 0")
```

Post intervention effect is not different from 0.
...
ggplot(model_data, aes(self_c, target_bfi_value, group = target_number_collapsed)) +
    ...
    labs(
      title = "Projection predicted by intervention (target order)",
      x = "BFI responses for self",
      y = "BFI responses for target"
      ) +
    scale_linetype_manual("Target order",
                        breaks = c("target_1", "target_2"), 
                       labels = c("Target 1 (pre)",
                                  "Target 2 (post)"),
                       values = c("solid",
                                  "dashed"))
```
...
ggplot(effects_data_threat, aes(self_c, fit, group = threat_c)) +
    ...
    theme_minimal(base_size = 13) +
    theme(legend.key.size = unit(1, "cm")) +
  scale_linetype_manual("Perceived \nGroup Threat",
                        breaks = c(-1.3, 0, 1.3), 
                       labels = c("Low",
                                  "Average",
                                  "High"),
                       values = c("solid",
                                  "dashed",
                                  "dotted")) +
    labs(title = "Projection by perceived group threat",
       x = "BFI responses for self",
       y = "BFI responses for target")
```
...
```{r}
targ_1_data <- model_data %>% 
  filter(target_number_collapsed == "target_1")

mod_targ1 <- lmer(target_bfi_value ~ self_c*condition_order + (self_c |sub_id), data = targ_1_data)
...
```{r}
targ_2_data <- model_data %>% 
  filter(target_number_collapsed == "target_2")

mod_targ2 <- lmer(target_bfi_value ~ self_c*condition_order + (self_c |sub_id), data = targ_2_data)
...
ggplot(model_data, aes(self_c, 
  ...
  scale_linetype_manual("Target order",
                       ...
                       values = c("solid",
                                  "dashed")) +
    labs(title = "Projection by intervention (target order) and target gender",
       x = "BFI responses for self",
       y = "BFI responses for target")
```
...
ggplot(lib_only, aes(self_c, 
  ...
  scale_linetype_manual("Target order",
                       ...
                       values = c("solid",
                                  "dashed")) +
    labs(title = "Projection by intervention (target order) and target gender",
         subtitle = "Only includes participants who did not vote for Trump",
       x = "BFI responses for self",
       y = "BFI responses for target")
```
